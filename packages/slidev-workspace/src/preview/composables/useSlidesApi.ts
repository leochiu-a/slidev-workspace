...
import { getSlides, getSlideById, getSlidesBySourceDir, searchSlides, type SlideData } from '@/utils/getSlides'
...
export function useSlidesApi() {
  ...
  const getEntries = async (options: QueryOptions = {}): Promise<SlidesApiResponse<SlideData[]>> => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      // Apply filters
      if (options.search) {
        slides = searchSlides(options.search)
      }

      if (options.sourceDir) {
        slides = slides.filter(slide => slide.sourceDir.includes(options.sourceDir!))
      }
  ...
  const getEntry = async (id: string): Promise<SlidesApiResponse<SlideData | null>> => {
    loading.value = true
    error.value = null
  ...
  const getEntriesByCollection = async (sourceDir: string): Promise<SlidesApiResponse<SlideData[]>> => {
    return getEntries({ sourceDir })
  }

  const getEntriesByContentType = async (theme: string): Promise<SlidesApiResponse<SlideData[]>> => {
    return getEntries({ theme })
  }

  // Reactive computed properties
  const allSlides = computed(() => getSlides())
  const slidesCount = computed(() => allSlides.value.length)
  const availableThemes = computed(() => {
    const themes = new Set(allSlides.value.map(slide => slide.theme).filter(Boolean))
  ...
  const createLiveQuery = (options: Ref<QueryOptions>) => {
    return computed(() => {
      ...
      if (options.value.sourceDir) {
        slides = slides.filter(slide => slide.sourceDir.includes(options.value.sourceDir!))
      }
